...
function updateBoard(){
	<c:if test="${sessionScope.member.mem_Id==dto.mem_id}">
			...
			var query = "bbs_num="+bbs_num+"&page="+page;
	 		var url = "<%=cp%>/pet/petTalk/update.do?" + query;
			location.href=url;
	</c:if>
...
function deleteBoard(){
	<c:if test="${sessionScope.member.mem_Id=='admin' || sessionScope.member.mem_Id==dto.mem_id}">
   		...
   		var page = "${page}";
   		var query = "bbs_num="+bbs_num+"&page="+page;
   		var url = "<%=cp%>/pet/petTalk/delete.do?" + query;
...
function listPage(page) {
	var url="<%=cp%>/pet/petTalk/listReply.do";
	var bbs_num="${dto.bbs_num}";
...
function sendReply(){
	...
	$.ajax({
		type:"post"
		,url:"<%=cp%>/pet/petTalk/insertReply.do"
		,data:query
		,dataType:"json"
...
function deleteReply(reply_num, page) {
	...
	if(confirm("게시물을 삭제하시겠습니까 ? ")) {	
		var url="<%=cp%>/pet/petTalk/deleteReply.do";
