...
<head>
<meta charset="UTF-8">
<title>뉴스/신제품 - ${dto.subject}</title>
...
<c:if test="${sessionScope.member.mem_Id=='admin' || sessionScope.member.mem_Id==dto.mem_Id}">
    ...
    var url = "<%=cp%>/food/recipe_delete.do?" + query;
...
function countLike(bbs_num) {
	var url="<%=cp%>/food/recipe_likecnt.do";
...
<c:if test="${sessionScope.member.mem_Id==dto.mem_Id}">
    ...
    var page = "${page}";
    var query = "bbs_num="+bbs_num+"&page="+page;
    var url = "<%=cp%>/food/recipe_update.do?" + query;
...
function listPage(page) {
	var url="<%=cp%>/food/recipe_listReply.do";
	var bbs_num="${dto.bbs_num}";
...
function sendReply() {
	...
	$.ajax({
		type:"post"
		,url:"<%=cp%>/food/recipe_insertReply.do"
		,data:query
		,dataType:"json"
		,success:function(data) {
			$("#content").val("");
...
function deleteReply(reply_num, page) {
	...
	if(confirm("게시물을 삭제하시겠습니까 ? ")) {	
		var url="<%=cp%>/food/recipe_deleteReply.do";
        ...
        <div>
			    ...
			    <td colspan="2" align="left" style="padding-left: 5px;">
			       이전글 :
			         <c:if test="${not empty preReadDto}">
			              <a href="<%=cp%>/food/recipe_article.do?${query}&bbs_num=${preReadDto.bbs_num}">${preReadDto.subject}</a>
			        </c:if>
			    </td>
			</tr>
			...
			<tr height="35" style="border-bottom: 1px solid #cccccc;">
			    ...
			    다음글 :
			         <c:if test="${not empty nextReadDto}">
			              <a href="<%=cp%>/food/recipe_article.do?${query}&bbs_num=${nextReadDto.bbs_num}">${nextReadDto.subject}</a>
			        </c:if>
			    </td>
			</tr>
			</table>
			...
			<tr height="45">
			    <td width="300" align="left">
			       <button type="button" class="btn" onclick="javascript:location.href='<%=cp%>/food/recipe_reply.do?bbs_num=${dto.bbs_num}&page=${page}';">답변</button>
			       <c:if test="${sessionScope.member.mem_Id==dto.mem_Id}">				    
			    ...
			    <td align="right">
			        <button type="button" class="btn" onclick="javascript:location.href='<%=cp%>/food/recipe.do?${query}';">리스트</button>
			    </td>
			</tr>
			</table>
        </div>
