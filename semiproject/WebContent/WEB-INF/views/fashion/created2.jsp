...
<script type="text/javascript">
    function sendOk() {
    	...
    	var mode="${mode}";
    	if(mode=="created")
    		f.action="<%=cp%>/fashion/Fashion2_created_ok.do";
    	else if(mode=="update")
    		f.action="<%=cp%>/fashion/Fashion2_update_ok.do";
       	else if(mode=="reply")
       		f.action="<%=cp%>/fashion/Fashion2_reply_ok.do";
			  ...
			  <table style="width: 100%; margin: 0px auto; border-spacing: 0px;">
			     <tr height="45"> 
			      <td align="center" >
			        ...
			        <button type="button" class="btn" onclick="javascript:location.href='<%=cp%>/fashion/Fashion2.do';">${mode=='update'?'수정취소':'등록취소'}</button>
...
/* function submitContents(elClickedObj) {
    ...
    oEditors.getById["Fashion2_CONTENT"].exec("UPDATE_CONTENTS_FIELD", [ ]);
 
    // 에디터의 내용에 대한 값 검증은 이곳에서
    // document.getElementById("Fashion2_CONTENT").value를 이용해서 처리한다.
