...
<body>
	...
	<div>
	<!-- 로고 이미지 -->
	<img src="<%=cp%>/resource/img/m_news.png" style="margin: 10px">
		<table
		...
		<table
			...
			<c:forEach var="dto" items="${listNotice}">
				<tr align="center" bgcolor="#ffffff" height="35"
					...
					<td><span
						style="display: inline-block; width: 50px; height: 18px; line-height: 18px; background: #ED4C00; color: #FFFFFF;">공지</span></td>
					<td>${dto.category}</td>
					<td align="left" style="padding-left: 10px;"><c:forEach
							var="n" begin="1" end="${dto.depth}">
			               &nbsp;
			           </c:forEach> <c:if test="${dto.depth!=0}">└&nbsp;</c:if> <a
						href="${articleUrl}&bbs_num=${dto.bbs_num}">${dto.subject}<c:if
								test="${dto.reply!=0}">[${dto.reply}]</c:if></a> <c:if
							test="${dto.gap<1}">
			...
			<tr height="40">
				<td align="left" width="100">
					<button type="button" class="btn"
						onclick="javascript:location.href='<%=cp%>/it/news.do';">새로고침</button>
				</td>
				<td align="center">
				...
				<td align="right" width="100">
					<button type="button" class="btn"
						onclick="javascript:location.href='<%=cp%>/it/news_created.do';">글올리기</button>
				</td>
			</tr>
		</table>
	</div>
