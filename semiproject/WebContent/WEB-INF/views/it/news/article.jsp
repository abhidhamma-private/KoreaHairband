...
<c:if test="${sessionScope.member.mem_Id=='admin' || sessionScope.member.mem_Id==dto.mem_Id}">
    ...
    var url = "<%=cp%>/it/news_delete.do?" + query;
...
<c:if test="${sessionScope.member.mem_Id==dto.mem_Id}">
    ...
    var page = "${page}";
    var query = "bbs_num="+bbs_num+"&page="+page;
    var url = "<%=cp%>/it/news_update.do?" + query;
...
function listPage(page) {
	var url="<%=cp%>/it/news_listReply.do";
	var bbs_num="${dto.bbs_num}";
...
function deleteReply(reply_num, page) {
	...
	if(confirm("게시물을 삭제하시겠습니까 ? ")) {	
		var url="<%=cp%>/it/news_deleteReply.do";
        ...
        <div>
			    ...
			    <td colspan="2" align="left" style="padding-left: 5px;">
			       이전글 :
			         <c:if test="${not empty preReadDto}">
			              <a href="<%=cp%>/it/news_article.do?${query}&bbs_num=${preReadDto.bbs_num}">${preReadDto.subject}</a>
			        </c:if>
			    </td>
			</tr>
			...
			<tr height="35" style="border-bottom: 1px solid #cccccc;">
			    ...
			    다음글 :
			         <c:if test="${not empty nextReadDto}">
			              <a href="<%=cp%>/it/news_article.do?${query}&bbs_num=${nextReadDto.bbs_num}">${nextReadDto.subject}</a>
			        </c:if>
			    </td>
			</tr>
			</table>
			...
			<tr height="45">
			    <td width="300" align="left">
			       <button type="button" class="btn" onclick="javascript:location.href='<%=cp%>/it/news_reply.do?bbs_num=${dto.bbs_num}&page=${page}';">답변</button>
			       <c:if test="${sessionScope.member.mem_Id==dto.mem_Id}">				    
			    ...
			    <td align="right">
			        <button type="button" class="btn" onclick="javascript:location.href='<%=cp%>/it/news.do?${query}';">리스트</button>
			    </td>
			</tr>
			</table>
        </div>
