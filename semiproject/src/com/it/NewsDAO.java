...
public class NewsDAO {
	...
	public int insertNews(NewsDTO dto, String mode) {
		int result = 0;
		...
		try {
			sql = "SELECT it1_seq.NEXTVAL FROM dual";
	...
	public int updateOrderNo(int groupNum, int orderNo) {
		...
		sql = "UPDATE it1 SET orderNo=orderNo+1 WHERE groupNum = ? AND orderNo > ?";
	...
	public int dataCount() {
		...
		try {
			sql = "SELECT NVL(COUNT(*), 0) FROM it1";
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			if (rs.next())
				result = rs.getInt(1);
	...
	public int dataCount(String searchKey, String searchValue) {
		...
		try {
			if (searchKey.equals("created")) {
				...
				sql = "SELECT NVL(COUNT(*), 0) FROM it1 b JOIN member m ON b.mem_Id=m.mem_Id WHERE TO_CHAR(created, 'YYYYMMDD') = ?  ";
			} else if (searchKey.equals("mem_Name")) {
				sql = "SELECT NVL(COUNT(*), 0) FROM it1 b JOIN member m ON b.mem_Id=m.mem_Id WHERE INSTR(mem_Name, ?) = 1 ";
			} else {
				sql = "SELECT NVL(COUNT(*), 0) FROM it1 b JOIN member m ON b.mem_Id=m.mem_Id WHERE INSTR(" + searchKey
						+ ", ?) >= 1 ";
			}
	...
	// 공지사항에 체크된것만 리스트 출력
	public List<NewsDTO> listNotice() {
		List<NewsDTO> listNotice = new ArrayList<NewsDTO>();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			...
			sb.append("		FROM it1 b JOIN member m ON b.mem_id = m.mem_id ");
	...
	public List<NewsDTO> listNews(int start, int end) {
	...
	// 검색에서 리스트
	public List<NewsDTO> listNews(int start, int end, String searchKey, String searchValue) {
		List<NewsDTO> list = new ArrayList<NewsDTO>();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		StringBuffer sb = new StringBuffer();
	...
	public NewsDTO readNews(int bbs_num) {
		NewsDTO dto = null;
		NewsDAO dao = new NewsDAO();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	// 이전글
	public NewsDTO preReadNews(int groupNum, int orderNo, String searchKey, String searchValue) {
		NewsDTO dto = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		StringBuffer sb = new StringBuffer();
		...
		try {
			if (searchValue != null && searchValue.length() != 0) {
				...
				sb.append("SELECT ROWNUM, tb.* FROM ( ");
				sb.append("     SELECT bbs_num, subject FROM it1 b JOIN member m ON b.mem_Id=m.mem_Id ");
	...
	// 다음글
	public NewsDTO nextReadNews(int groupNum, int orderNo, String searchKey, String searchValue) {
		NewsDTO dto = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		StringBuffer sb = new StringBuffer();
	...
	public int updateHitCount(int bbs_num) {
		...
		sql = "UPDATE it1 SET hitCount=hitCount+1 WHERE bbs_num=?";
	...
	public int countLike(int bbs_num) {
		...
		try {
			sql = "SELECT NVL(COUNT(*), 0) FROM it1_like WHERE bbs_num=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, bbs_num);
	...
	public int updateLike(int bbs_num, String mem_Id) {
		...
		sql = "INSERT INTO it1_like(bbs_num, mem_Id) VALUES (?, ?)";
		try {
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, bbs_num);
	...
	public int updateNews(NewsDTO dto, String mem_Id) {
		int result = 0;
		PreparedStatement pstmt = null;
		String sql;

		sql = "UPDATE it1 SET subject=?, content=? ";
	...
	public int deleteNews(int bbs_num) {
		int result = 0;
		PreparedStatement pstmt = null;
		String sql;
		...
		try {
			// 좋아요 삭제
			sql = "DELETE FROM it1_like WHERE bbs_num = ? ";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, bbs_num);
			result = pstmt.executeUpdate();
			pstmt.close();
			...
			// 리플 삭제
			sql = "DELETE FROM it1_reply WHERE bbs_num = ? ";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, bbs_num);
			result = pstmt.executeUpdate();
			pstmt.close();
			...
			// 게시글 삭제
			sql = "DELETE FROM it1 WHERE bbs_num IN (SELECT bbs_num FROM it1 START WITH  bbs_num = ? CONNECT BY PRIOR bbs_num = parent)";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, bbs_num);
			result = pstmt.executeUpdate();
			pstmt.close();
	...
	// 게시물의 댓글 및 답글 추가
	public int insertReply(NewsReplyDTO dto) {
		int result = 0;
		PreparedStatement pstmt = null;
		StringBuffer sb = new StringBuffer();

		try {
			sb.append("INSERT INTO it1_reply(reply_num, bbs_num, mem_Id, content) ");
			sb.append(" VALUES (it1_reply_seq.NEXTVAL, ?, ?, ?)");
	...
	public int dataCountReply(int bbs_num) {
		...
		try {
			sql = "SELECT NVL(COUNT(*), 0) FROM it1_reply WHERE bbs_num=? ";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, bbs_num);
	...
	// 게시물 댓글 리스트
	public List<NewsReplyDTO> listReply(int bbs_num, int start, int end) {
		List<NewsReplyDTO> list = new ArrayList<>();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		StringBuffer sb = new StringBuffer();
		...
		try {
			...
			sb.append("			FROM it1_reply r  ");
			sb.append("			JOIN it1 b ON r.bbs_num = b.bbs_num  ");
			...
			while (rs.next()) {
				NewsReplyDTO dto = new NewsReplyDTO();
