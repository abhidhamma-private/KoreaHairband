...
public class Fashion2DAO {
	private Connection conn = DBConn.getConnection();

	public int insertFashion2(Fashion2DTO dto, String mode) {
		int result = 0;
		...
		try {
			sql = "SELECT fashion2_seq.NEXTVAL FROM dual";
	...
	public int updateOrderNo(int groupNum, int orderNo) {
		...
		sql = "UPDATE fashion2 SET orderNo=orderNo+1 WHERE groupNum = ? AND orderNo > ?";
	...
	public int dataCount() {
		...
		try {
			sql = "SELECT NVL(COUNT(*), 0) FROM fashion2";
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			if (rs.next())
				result = rs.getInt(1);
	...
	public int dataCount(String searchKey, String searchValue) {
		...
		try {
			if (searchKey.equals("created")) {
				...
				sql = "SELECT NVL(COUNT(*), 0) FROM fashion2 b JOIN member m ON b.mem_Id=m.mem_Id WHERE TO_CHAR(created, 'YYYYMMDD') = ?  ";
			} else if (searchKey.equals("mem_Name")) {
				sql = "SELECT NVL(COUNT(*), 0) FROM fashion2 b JOIN member m ON b.mem_Id=m.mem_Id WHERE INSTR(mem_Name, ?) = 1 ";
			} else {
				sql = "SELECT NVL(COUNT(*), 0) FROM fashion2 b JOIN member m ON b.mem_Id=m.mem_Id WHERE INSTR(" + searchKey
						+ ", ?) >= 1 ";
			}
	...
	// 공지사항에 체크된것만 리스트 출력
	public List<Fashion2DTO> listNotice() {
		List<Fashion2DTO> listNotice = new ArrayList<Fashion2DTO>();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			...
			sb.append("		FROM fashion2 b JOIN member m ON b.mem_id = m.mem_id ");
	...
	public List<Fashion2DTO> listFashion2(int start, int end) {
		List<Fashion2DTO> list = new ArrayList<Fashion2DTO>();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		StringBuffer sb = new StringBuffer();
		Fashion2DAO dao = new Fashion2DAO();
	...
	// 검색에서 리스트
	public List<Fashion2DTO> listFashion2(int start, int end, String searchKey, String searchValue) {
		List<Fashion2DTO> list = new ArrayList<Fashion2DTO>();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		StringBuffer sb = new StringBuffer();
	...
	public Fashion2DTO readFashion2(int bbs_num) {
		Fashion2DTO dto = null;
		Fashion2DAO dao = new Fashion2DAO();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		StringBuffer sb = new StringBuffer();
	...
	// 이전글
	public Fashion2DTO preReadFashion2(int groupNum, int orderNo, String searchKey, String searchValue) {
		Fashion2DTO dto = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		StringBuffer sb = new StringBuffer();
		...
		try {
			if (searchValue != null && searchValue.length() != 0) {
				...
				sb.append("SELECT ROWNUM, tb.* FROM ( ");
				sb.append("     SELECT bbs_num, subject FROM fashion2 b JOIN member m ON b.mem_Id=m.mem_Id ");
	...
	// 다음글
	public Fashion2DTO nextReadFashion2(int groupNum, int orderNo, String searchKey, String searchValue) {
		Fashion2DTO dto = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		StringBuffer sb = new StringBuffer();
	...
	public int updateHitCount(int bbs_num) {
		...
		sql = "UPDATE fashion2 SET hitCount=hitCount+1 WHERE bbs_num=?";
	...
	public int countLike(int bbs_num) {
		...
		try {
			sql = "SELECT NVL(COUNT(*), 0) FROM fashion2_like WHERE bbs_num=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, bbs_num);
	...
	public int updateLike(int bbs_num, String mem_Id) {
		...
		sql = "INSERT INTO fashion2_like(bbs_num, mem_Id) VALUES (?, ?)";
		try {
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, bbs_num);
	...
	public int updateFashion2(Fashion2DTO dto, String mem_Id) {
		int result = 0;
		PreparedStatement pstmt = null;
		String sql;

		sql = "UPDATE fashion2 SET subject=?, content=? ";
	...
	public int deleteFashion2(int bbs_num) {
		int result = 0;
		PreparedStatement pstmt = null;
		String sql;
		...
		try {
			// 좋아요 삭제
			sql = "DELETE FROM fashion2_like WHERE bbs_num = ? ";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, bbs_num);
			result = pstmt.executeUpdate();
			pstmt.close();
			...
			// 리플 삭제
			sql = "DELETE FROM fashion2_reply WHERE bbs_num = ? ";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, bbs_num);
			result = pstmt.executeUpdate();
			pstmt.close();
			...
			// 게시글 삭제
			sql = "DELETE FROM fashion2 WHERE bbs_num IN (SELECT bbs_num FROM fashion2 START WfashionH  bbs_num = ? CONNECT BY PRIOR bbs_num = parent)";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, bbs_num);
			result = pstmt.executeUpdate();
			pstmt.close();
	...
	// 게시물의 댓글 및 답글 추가
	public int insertReply(Fashion2ReplyDTO dto) {
		int result = 0;
		PreparedStatement pstmt = null;
		StringBuffer sb = new StringBuffer();

		try {
			sb.append("INSERT INTO fashion2_reply(reply_num, bbs_num, mem_Id, content) ");
			sb.append(" VALUES (fashion2_reply_seq.NEXTVAL, ?, ?, ?)");
	...
	public int dataCountReply(int bbs_num) {
		...
		try {
			sql = "SELECT NVL(COUNT(*), 0) FROM fashion2_reply WHERE bbs_num=? ";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, bbs_num);
	...
	// 게시물 댓글 리스트
	public List<Fashion2ReplyDTO> listReply(int bbs_num, int start, int end) {
		List<Fashion2ReplyDTO> list = new ArrayList<>();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		StringBuffer sb = new StringBuffer();
		...
		try {
			...
			sb.append("			FROM fashion2_reply r  ");
			sb.append("			JOIN fashion2 b ON r.bbs_num = b.bbs_num  ");
			...
			while (rs.next()) {
				Fashion2ReplyDTO dto = new Fashion2ReplyDTO();
	...
	public int deleteReply(int reply_num, String mem_Id) {
		...
		sql = "DELETE FROM fashion2_reply ";
		sql += "  WHERE reply_num = ? ";
