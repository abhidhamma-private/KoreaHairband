...
public class BoardDAO {
	...
	public int insertBoard(BoardDTO dto, String mode) {
		...
		try {
			sql = "SELECT food2_seq.NEXTVAL FROM dual";
	...
	public int updateOrderNo(int groupNum, int orderNo) {
		...
		sql = "UPDATE food2 SET orderNo=orderNo+1 WHERE groupNum = ? AND orderNo > ?";
	...
	public int dataCount() {
		...
		try {
			sql = "SELECT NVL(COUNT(*), 0) FROM food2";
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			if (rs.next())
				result = rs.getInt(1);
	...
	public int dataCount(String searchKey, String searchValue) {
		...
		try {
			if (searchKey.equals("created")) {
				...
				sql = "SELECT NVL(COUNT(*), 0) FROM food2 b JOIN member m ON b.mem_Id=m.mem_Id WHERE TO_CHAR(created, 'YYYYMMDD') = ?  ";
			} else if (searchKey.equals("mem_Name")) {
				sql = "SELECT NVL(COUNT(*), 0) FROM food2 b JOIN member m ON b.mem_Id=m.mem_Id WHERE INSTR(mem_Name, ?) = 1 ";
			} else {
				sql = "SELECT NVL(COUNT(*), 0) FROM food2 b JOIN member m ON b.mem_Id=m.mem_Id WHERE INSTR(" + searchKey
						+ ", ?) >= 1 ";
			}
	...
	public List<BoardDTO> listNotice() {
		List<BoardDTO> listNotice = new ArrayList<BoardDTO>();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			...
			sb.append("		FROM food2 b JOIN member m ON b.mem_id = m.mem_id ");
	...
	public BoardDTO preReadBoard(int groupNum, int orderNo, String searchKey, String searchValue) {
		...
		try {
			if (searchValue != null && searchValue.length() != 0) {
				...
				sb.append("SELECT ROWNUM, tb.* FROM ( ");
				sb.append("     SELECT bbs_num, subject FROM food2 b JOIN member m ON b.mem_Id=m.mem_Id ");
	...
	public int updateHitCount(int bbs_num) {
		...
		sql = "UPDATE food2 SET hitCount=hitCount+1 WHERE bbs_num=?";
	...
	public int countLike(int bbs_num) {
		...
		try {
			sql = "SELECT NVL(COUNT(*), 0) FROM food2_like WHERE bbs_num=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, bbs_num);
	...
	public int updateLike(int bbs_num, String mem_Id) {
		...
		sql = "INSERT INTO food2_like(bbs_num, mem_Id) VALUES (?, ?)";
		try {
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, bbs_num);
	...
	public int updateBoard(BoardDTO dto, String mem_Id) {
		...
		sql = "UPDATE food2 SET subject=?, content=? ";
	...
	public int deleteBoard(int bbs_num) {
		...
		try {
			// 좋아요 삭제
			sql = "DELETE FROM food2_like WHERE bbs_num = ? ";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, bbs_num);
			result = pstmt.executeUpdate();
			pstmt.close();
			...
			// 리플 삭제
			sql = "DELETE FROM food2_reply WHERE bbs_num = ? ";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, bbs_num);
			result = pstmt.executeUpdate();
			pstmt.close();
			...
			// 게시글 삭제
			sql = "DELETE FROM food2 WHERE bbs_num IN (SELECT bbs_num FROM food2 START WITH  bbs_num = ? CONNECT BY PRIOR bbs_num = parent)";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, bbs_num);
			result = pstmt.executeUpdate();
			pstmt.close();
	...
	public int insertReply(BoardReplyDTO dto) {
		...
		try {
			sb.append("INSERT INTO food2_reply(reply_num, bbs_num, mem_Id, content) ");
			sb.append(" VALUES (food2_reply_seq.NEXTVAL, ?, ?, ?)");
	...
	public int dataCountReply(int bbs_num) {
		...
		try {
			sql = "SELECT NVL(COUNT(*), 0) FROM food2_reply WHERE bbs_num=? ";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, bbs_num);
	...
	public List<BoardReplyDTO> listReply(int bbs_num, int start, int end) {
		...
		try {
			...
			sb.append("			FROM food2_reply r  ");
			sb.append("			JOIN food2 b ON r.bbs_num = b.bbs_num  ");
